...
def get_all_anim_curves(obj, attr=None):
    ...
    obj_attr = obj if not attr else '{}.{}'.format(obj, attr)

    anim_curves = []
    layers_obj = layers.get_affected_anim_layer(obj=obj_attr)
    if layers_obj:
        for layer in layers_obj:
            anim_curves.extend(layers.get_obj_curves_from_layer(layer=layer, obj=obj, attr=attr))

    curves_not_in_later = cmds.listConnections(obj_attr, t='animCurve')
